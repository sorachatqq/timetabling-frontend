...
import { JSXElementConstructor, Key, PromiseLikeOfReactNode, ReactElement, ReactNode, ReactPortal, JSXElementConstructor, Key, PromiseLikeOfReactNode, ReactElement, ReactNode, JSXElementConstructor, Key, PromiseLikeOfReactNode, ReactElement, ReactNode, ReactPortal, JSXElementConstructor, Key, PromiseLikeOfReactNode, ReactElement, ReactNode, ReactPortal, useEffect, useState } from 'react';
...
export default function ResultPage() {
  ...
  const { id } = useParams();
  const [outputDataAPI, setOutputData] = useState<any>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (id) {
        try {
          const response = await fetch(`https://unitime-api.aomdev.com/api/v1/task/${id}`);
          const result = await response.json();
          setOutputData(result.data.output.examtt); // Update this line to match the correct data structure
          console.log('%capp/result/[id]/page.tsx:77 object', 'color: #007acc;', result.data.output.examtt);
        } catch (error) {
  ...
  useEffect(() => {
    if (outputData) {
      console.log('%capp/result/[id]/page.tsx:77 object', 'color: #007acc;', outputData);
    }
  }, [outputData]);
  ...
  const mappedExams = examData.map((exam: {
    ...
    return {
      ...
      instructor: instructors.map(instr => instr.name).join(', '),
      student: exam.enrl,
